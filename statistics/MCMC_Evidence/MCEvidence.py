...
cosmo_params_list=['omegabh2','omegach2','theta','tau','omegak','mnu','MAx','w','wa',
                       'nnu','delta_DMDE','yhe','alpha1','deltazrei','Alens','Alensf','fdm','logA','ns','nrun',
...
def iscosmo_param(p,cosmo_params=None):
    ...
    '''        
    if not cosmo_params is None: cosmo_params_list.extend(cosmo_params)
...
def params_info(fname,cosmo=False, volumes={}):
    ...
    if glob.glob('{}*.ranges'.format(fname)):
        ...
        parName=[name.decode('UTF-8') for name in par['name']]
        parMin=par['min']
        parMax=par['max']
